...
@section('content')
<div class="container">
    <h4 class="grey-text text-darken-2 center">Verify Overtimes</h4>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <div class="row mb-0">
                                                ...
                                                <div class="col">
                                                    <form action="{{route('overtime.postverify',$ot_record->id)}}" method="POST">
                                                        @csrf()
                                                        <input type="hidden" id="staff_id" name="staff_id" value="{{$ot_record->staff_id}}" />
                                                        <input type="hidden" id="status" name="status" value="Yes" />
                                                        <button type="submit" title="Verify" class="btn btn-floating btn-small waves=effect waves-light green"><i class="material-icons">done</i></button>
                                                       </form>
                                                </div>                                          
                                                <div class="col">
                                                    <form action="{{route('overtime.rejectverify',$ot_record->id)}}" method="POST">
                                                        @csrf()
